...
class ReservoirCell(keras.layers.Layer):
    ...
    def __init__(self, units,embeddings,vocab_size,embedding_size,batch_size,
                 input_scaling=1., spectral_radius=0.99, leaky=1,
                 connectivity_input=10, connectivity_recurrent=10,l2_reg_lambda = 0.0, is_Embedding_Needed = False,trainable = True,overlap_needed = True,time_needed = True,pooling = 'max',hidden_num = 10, extend_feature_dim = 10
                 **kwargs):
    ...
    def Time_Embedding(self,time_size):
        batch_size=self.batch_size
        seq_len = self.vocab_size
        time_j = 1. / tf.pow(10000., 2 * tf.range(time_size, dtype=tf.float32) / time_size)
        time_j = tf.expand_dims(time_j, 0)

        time_i=tf.range(tf.cast(seq_len,tf.float32), dtype=tf.float32)
        time_i=tf.expand_dims(time_i,1)
        time_ij = tf.matmul(time_i, time_j)
        time_embedding = time_ij
    ...
    def add_embeddings(self):
        with tf.name_scope("embedding"):
            if self.is_Embedding_Needed:
                ...
                W_t=tf.Variable(self.Time_Embedding(self.embedding_size),name = 'W',trainable = self.trainable)
            else:
                W = tf.Variable(tf.random_uniform([self.vocab_size, self.embedding_size], -1.0, 1.0),name="W",trainable = self.trainable)
            self.embedding_W = W
            self.embedding_W_t=W_t
